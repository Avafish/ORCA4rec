...
import numpy as np
import random
from utils import get_data, get_optimizer_scheduler, count_params, count_trainable_params
...
parser.add_argument('--dataset', default='wikipedia.txt')
...
parser.add_argument('--maxsamples', default=20, type=int)
...
for ep in range(args.epochs + args.predictor_epochs):
    ...
    if ep % args.validation_freq == 0 or ep == args.epochs + args.predictor_epochs - 1:
        val_loss, val_score = evaluate(args, model, valid_loader, loss, n_val, topks, decoder, transform)
    ...
    if ep == args.epochs + args.predictor_epochs - 1:
        ...
        test_loss, test_score = evaluate(args, test_model, test_loader, loss, n_test, topks, decoder, transform)
